...
void initialize_internal_node(void* node) {
    ...
    *internal_node_num_keys(node) = 0;
    ...
    // child to an invalid page number when initializing the node, we may end up with 0 as the
    // node's right child, which makes the node a parent of the root.
...
uint32_t* internal_node_child(void* node, uint32_t child_num) {
    ...
    if (child_num > num_keys) {
        ...
        if (*right_child == INVALID_PAGE_NUM) {
            printf("Tried to access right child of a node, but was invalid page\n");
            exit(EXIT_FAILURE);
        }
        return right_child;
    } else {
...
Cursor* leaf_node_find(Table* table, uint32_t page_num, uint32_t key) {
    ...
    while (one_past_max_index != min_index) {
        ...
        uint32_t key_at_inex = *leaf_node_key(node, index);
        if (key == key_at_inex) {
...
void create_new_root(Table* table, uint32_t right_child_page_num) {
    ...
    // Address of righ child passed in.
...
void internal_node_split_and_insert(Table* table, uint32_t parent_page_num, uint32_t child_page_num) {
    ...
    created. If it does not, we have to insert the newly created node into its parent after the old node's
    key have been transfered over. We are not able to do this if the newly created node's parent is not a 
    newly initialized root node, because in that case its parent may have existing keys aside from our old
    which we are splitting. If that is true, we need to find a place for our newly created node in its parent,
   ...
   *internal_node_right_child(old_node) = INVALID_PAGE_NUM;
   ...
   // Set child before middle key, which is now the highest key, to be node's right child, and decrement
   // number of keys.
   *internal_node_right_child(old_node) = *internal_node_child(old_node, *old_num_keys - 1);
   (*old_num_keys)--;

   // Determine which of the two nodes after the split should contain the child to be insertes, and insert
   // the child.
...
void internal_node_insert(Table* table, uint32_t parent_page_num, uint32_t child_page_num) {
    ...
    void* right_child = get_page(table->pager, right_child_page_num);
    // If we are at the max number of cells for a node, we cannot increment before splitting.
...
void leaf_node_split_and_insert(Cursor* cursor, uint32_t key, Row* value) {
    ...
    *leaf_node_next_leaf(old_node) = new_page_num;
    ...
    // All existing keys plus new key should be devided evenly between old (left) and new (right) nodes.
    // Starting from the right, move each key to correct position.
    for (int32_t i = LEAF_NODE_MAX_CELLS; i >= 0; i--) {
        /* void* destination_node = new_node;
        ...
        } else if (i > 0 && i > cursor->cell_num) {
            memcpy(destination, leaf_node_cell(old_node, i - 1), LEAF_NODE_CELL_SIZE);
        } else if (i < LEAF_NODE_MAX_CELLS) {
